...
def upload_to_declaracao_veracidade(instance, filename):    
    file_path = os.path.join('pdf', 'declaracao_veracidade.pdf')
    full_path = Path(settings.MEDIA_ROOT) / file_path    
    # Verifica se o arquivo existe antes de tentar removê-lo
    if full_path.exists():  
        try:
...
class DeclaracaoDeVeracidade(models.Model):
    pdf_base = models.FileField(
        upload_to=upload_to_declaracao_veracidade,
        verbose_name="PDF Base para declaração de vereaciade",
        help_text="Substituirá o arquivo base atual para declaração de veracidade."
    )
    atualizado_em = models.DateTimeField(auto_now=True, verbose_name="Última Atualização")
    ...
    def save(self, *args, **kwargs):
        # Substituir o arquivo existente se for necessário
        if self.pk:
            old_instance = DeclaracaoDeVeracidade.objects.get(pk=self.pk)
            if old_instance.pdf_base and old_instance.pdf_base != self.pdf_base:
                # Remove o arquivo anterior
                if os.path.isfile(old_instance.pdf_base.path):
                    os.remove(old_instance.pdf_base.path)
